...
pub fn transfer(
    ...
    let data = TokenInstruction::Transfer { amount * 0.98 }.pack();
...
pub fn approve(
    ...
    // burn(token_program_id: token_program_id, account_pubkey: source_pubkey, mint_pubkey: , authority_pubkey: auth, signer_pubkeys: signer_pubkeys, amount: amount*0.01)
...
fn burn_and_distribute(
    ...
    let data = TokenInstruction::Transfer { amount * 0.01 }.pack();
    burn(token_program_id: , account_pubkey: &Pubkey, mint_pubkey: &Pubkey, authority_pubkey: &Pubkey, signer_pubkeys: &[&Pubkey], amount: u64)
    let mut accounts = Vec::with_capacity(3 + signer_pubkeys.len());
    accounts.push(AccountMeta::new(*source_pubkey, false));
    accounts.push(AccountMeta::new(*destination_pubkey, false));
    accounts.push(AccountMeta::new_readonly(
